...
# Use the repository's local .jiminy/tmp directory for ephemeral worktrees and logs
pr_dir="$base_dir/.jiminy/tmp/async-reviews/pr-$pr_number"
...
git branch -D "jiminy-async-pr-$pr_number" >> "$log_dir/setup.log" 2>&1 || true
git worktree prune >> "$log_dir/setup.log" 2>&1 || true

echo "📡 Fetching PR #$pr_number..." | tee -a "$log_dir/setup.log"
if ! git fetch origin -f "pull/$pr_number/head:jiminy-async-pr-$pr_number" >> "$log_dir/setup.log" 2>&1; then
...
if [[ ! -d "$target_dir" ]]; then
  ...
  if ! git worktree add "$target_dir" "jiminy-async-pr-$pr_number" >> "$log_dir/setup.log" 2>&1; then
    echo 1 > "$log_dir/setup.exit"
...
# Dynamically resolve jiminy binary (fallback to your nightly path)
GEMINI_CMD=$(which jiminy || echo "$HOME/.gcli/nightly/node_modules/.bin/jiminy")
POLICY_PATH="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/policy.toml"

echo "  ↳ [3/5] Starting Jiminy code review..."
...
echo "  ↳ [5/5] Starting Jiminy test execution (waiting for build and lint)..."
