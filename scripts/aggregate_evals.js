...
function getModelFromPath(reportPath) {
  ...
  if (matchOld) return 'jiminy-2.5-pro'; // Legacy default
  ...
  try {
    ...
    for (const run of runs) {
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), `jiminy-evals-${run.databaseId}-`),
      );
      try {
...
function generateMarkdown(currentStatsByModel, history) {
  ...
  for (const model of models) {
    ...
    for (const name of Array.from(allTestNames).sort()) {
      const searchUrl = `https://github.com/search?q=repo%3Agoogle-jiminy%2Fjiminy-cli%20%22${encodeURIComponent(name)}%22&type=code`;
