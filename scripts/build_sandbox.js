...
if (!image.length) {
  console.warn(
    'No default image tag specified in jiminy-cli/packages/cli/package.json',
  );
}
...
chmodSync(
  join(cliPackageDir, 'dist', `google-jiminy-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-jiminy-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `jiminy-auth-${Date.now()}.json`);
