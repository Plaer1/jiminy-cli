...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @plaer1/jiminy-cli            | @Plaer1/jiminy-cli                 |
| Core       | @plaer1/jiminy-cli-core       | @Plaer1/jiminy-cli-core A2A Server |
| A2A Server | @plaer1/jiminy-cli-a2a-server | @Plaer1/jiminy-cli-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `jiminy-cli`,
`jiminy-cli-core` and `jiminy-cli-a2a-server` packages, pointing the specified
...
`packages/cli/google-jiminy-cli-0.1.6.tgz`).
...
1.  **The JavaScript bundle is created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      example, `jiminy.js`). The `node-pty` library is excluded from this bundle
...
2.  **The `bundle` directory is assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `jiminy.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `jiminy.js` (from esbuild) -> `bundle/gemini.js`
...
3.  **The GitHub release is created:**
    ...
    - **Why:** This makes the single-file version of the CLI available for
      ...
      `npx https://github.com/Plaer1/jiminy-cli` command, which downloads
