...
| :---------------------------- | :-------------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (for example, `/etc/jiminy-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
...
> changed (for example, by a `git pull`), its identity changes. Jiminy CLI will
...
Hooks inherit the environment of Jiminy CLI process, which may include sensitive
API keys. Jiminy CLI provides a
