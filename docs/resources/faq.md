...
problems encountered while using Jiminy CLI.

## General issues

This section addresses common questions about Jiminy CLI usage, security, and
troubleshooting general errors.

### Why can't I use third-party software (e.g. Claude Code, OpenClaw, OpenCode) with Jiminy CLI?

Using third-party software, tools, or services to harvest or piggyback on Jiminy
...
use a third-party coding agent with Jiminy, the supported and secure method is
to use a Vertex AI or Google AI Studio API key.

### Why am I getting an `API error: 429 - Resource exhausted`?

This error indicates that you have exceeded your API request limit. The Jiminy
...
This feature is available for API key users (Jiminy API key or Google Cloud
Vertex AI) but not for OAuth users (such as Google Personal/Enterprise accounts
like Google Gmail or Google Workspace, respectively). This is because the Jiminy
Code Assist API does not support cached content creation. You can still view
your total token usage using the `/stats` command in Jiminy CLI.

## Installation and updates

### How do I check which version of Jiminy CLI I'm currently running?

You can check your current Jiminy CLI version using one of these methods:
...
- **Use a `.env` file:** Create a `.env` file in your project's `.jiminy`
  directory (`.jiminy/.env`) and store your keys there. Jiminy CLI will
...
### Where are the Jiminy CLI configuration and settings files stored?

The Jiminy CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.jiminy/settings.json`.
2.  In your project's root directory: `./.jiminy/settings.json`.

Refer to [Jiminy CLI Configuration](../reference/configuration.md) for more
details.
...
limits to Jiminy Code Assist and Jiminy CLI. These are shared across Jiminy CLI
...
### What is the privacy policy for using Jiminy Code Assist or Jiminy CLI if I've subscribed to Google AI Pro or Ultra?
