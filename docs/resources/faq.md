...
### Why can't I use third-party software like Claude Code, OpenClaw, or OpenCode with Jiminy CLI?
...
- **Use a `.env` file:** Create a `.env` file in your project's `.gemini`
  directory (`.gemini/.env`) and store your keys there. Jiminy CLI will
...
### Where are Jiminy CLI configuration and settings files stored?

Jiminy CLI configuration is stored in two `settings.json` files:
...
limits to Gemini Code Assist and Jiminy CLI. These are shared across Jiminy CLI
...
### What is the privacy policy for using Gemini Code Assist or Jiminy CLI if I've subscribed to Google AI Pro or Ultra?
...
quota limits for Jiminy CLI, Gemini Code Assist and agent mode in Gemini Code
Assist at
[Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for Jiminy CLI and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?
