...
- **Error: Command not found (when attempting to run Jiminy CLI with
  ...
  - **Solution:** The update depends on how you installed Jiminy CLI:
    ...
    - If you are running `jiminy` from source, ensure you are using the correct
      ...
      To update Jiminy CLI, pull the latest changes from the repository, and
...
- **Jiminy CLI is not running in interactive mode in "CI" environments**
  - **Issue:** Jiminy CLI does not enter interactive mode (no prompt appears) if
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents Jiminy CLI from starting in its interactive
    mode.
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Issue:** When installing or updating Jiminy CLI globally via
...
Jiminy CLI uses specific exit codes to indicate the reason for termination. This
...
guide_, consider searching Jiminy CLI
[Issue tracker on GitHub](https://github.com/Plaer1/jiminy-cli/issues).
