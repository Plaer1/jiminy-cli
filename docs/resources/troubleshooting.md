...
- **Error:
  `You must be a named user on your organization's Jiminy Code Assist Standard edition subscription to use this service. Please contact your administrator to request an entitlement to Jiminy Code Assist Standard edition.`**
  - **Cause:** This error might occur if Jiminy CLI detects the
  ...
  - **Solution:**
    ...
    - **Organizational Users:** Contact your Google Cloud administrator to be
      added to your organization's Jiminy Code Assist subscription.
...
- **Error: Command not found (when attempting to run Jiminy CLI with
  `jiminy`).**
  - **Cause:** Jiminy CLI is not correctly installed or it is not in your
    system's `PATH`.
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Jiminy CLI may attempt operations
...
- **Jiminy CLI is not running in interactive mode in "CI" environments**
  - **Issue:** The Jiminy CLI does not enter interactive mode (no prompt
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the Jiminy CLI from starting in its
    interactive mode.
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to
    ...
    `env -u CI_TOKEN jiminy`

- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for jiminy-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with jiminy-cli behavior.
  - **Solution:** Use a `.jiminy/.env` file instead, or configure the
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Cause:** These warnings occur because some dependencies (or their
    ...
    versions. Since Jiminy CLI requires Node.js 20 or higher, the platform's
...
The Jiminy CLI uses specific exit codes to indicate the reason for termination.
