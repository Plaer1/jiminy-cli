...
Unlike general context files ([`GEMINI.md`](./jiminy-md.md)), which provide
persistent workspace-wide background, Skills represent **on-demand expertise**.
This allows Jiminy to maintain a vast library of specialized capabilities—such
as security auditing, cloud deployments, or codebase migrations—without
cluttering the model's immediate context window.

Jiminy autonomously decides when to employ a skill based on your request and the
...
1.  **Workspace Skills**: Located in `.jiminy/skills/` or the `.agents/skills/`
    alias. Workspace skills are typically committed to version control and
    shared with the team.
2.  **User Skills**: Located in `~/.jiminy/skills/` or the `~/.agents/skills/`
...
precedence over the `.jiminy/skills/` directory. This generic alias provides an
...
# Discovers skills (SKILL.md or */SKILL.md) and creates symlinks in ~/.jiminy/skills
# (or ~/.agents/skills)
jiminy skills link /path/to/my-skills-repo

# Link to the workspace scope (.jiminy/skills or .agents/skills)
jiminy skills link /path/to/my-skills-repo --scope workspace

# Install a skill from a Git repository, local directory, or zipped skill file (.skill)
# Uses the user scope by default (~/.jiminy/skills or ~/.agents/skills)
jiminy skills install https://github.com/user/repo.git
jiminy skills install /path/to/local/skill
jiminy skills install /path/to/local/my-expertise.skill

# Install a specific skill from a monorepo or subdirectory using --path
jiminy skills install https://github.com/my-org/my-skills.git --path skills/frontend-design

# Install to the workspace scope (.jiminy/skills or .agents/skills)
jiminy skills install /path/to/skill --scope workspace

# Uninstall a skill by name
jiminy skills uninstall my-expertise --scope workspace

# Enable a skill (globally)
jiminy skills enable my-expertise

# Disable a skill. Can use --scope to specify workspace or user (defaults to workspace)
jiminy skills disable my-expertise --scope workspace
```
...
1.  **Discovery**: At the start of a session, Jiminy CLI scans the discovery
    ...
    system prompt.
2.  **Activation**: When Jiminy identifies a task matching a skill's
...
Once a skill is activated (typically by Jiminy identifying a task that matches
