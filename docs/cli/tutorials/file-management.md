# File management with Jiminy CLI

Explore, analyze, and modify your codebase using Jiminy CLI. In this guide,
you'll learn how to provide Jiminy CLI with files and directories, modify and
create files, and control what Jiminy CLI can see.
...
Jiminy CLI will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that Jiminy CLI uses a file,
...
If you _don't_ know the exact file path, you can ask Jiminy CLI to find it for
...
Jiminy uses the `glob` or `list_directory` tools to search your project
...
Once Jiminy CLI has context, you can direct it to make specific edits. The agent
...
Jiminy CLI uses the `replace` tool to propose a targeted code change.
...
Jiminy CLI uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

Jiminy CLI prioritizes safety. Before any file is modified, it presents a
...
Jiminy CLI uses the `run_shell_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what Jiminy sees

By default, Jiminy CLI respects your `.gitignore` file. It won't read or search
...
`.jiminyignore` file in your project root.

**Example `.jiminyignore`:**
