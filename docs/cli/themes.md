...
Jiminy CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Jiminy CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Jiminy CLI:
...
Selected themes are saved in Jiminy CLI's
...
Jiminy CLI lets you create your own custom color themes by specifying them in
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, Jiminy CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in Jiminy CLI. Your custom
