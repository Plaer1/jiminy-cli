...
which projects can use the full capabilities of Jiminy CLI. It prevents
...
Once the feature is enabled, the first time you run Jiminy CLI from a folder, a
...
Before you make a choice, Jiminy CLI performs a **discovery phase** to scan the
...
When a folder is **untrusted**, Jiminy CLI runs in a restricted "safe mode" to
...
Granting trust to a folder unlocks the full functionality of Jiminy CLI for that
workspace.

## Headless and automated environments

When running Jiminy CLI in a headless environment (for example, a CI/CD
...
review the
[Jiminy CLI trust guidance for GitHub Actions](https://github.com/Plaer1/jiminy-cli/blob/main/docs/trust-guidance.md).
