...
Jiminy CLI can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate Jiminy CLI with an IDE:

1.  **VS Code companion extension**: Install the "Jiminy CLI Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like Jiminy CLI.

## VS Code companion extension

The **Jiminy CLI Companion extension** grants Jiminy CLI direct access to your
...
- **VS Code commands:** You can access Jiminy CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Jiminy CLI: Run`: Starts a new Jiminy CLI session in the integrated
    terminal.
  - `Jiminy CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Jiminy CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Jiminy CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Jiminy CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Jiminy CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.jiminy-cli-vscode-ide-companion).
- **For VS Code forks:** To support forks of VS Code, the extension is also
  published on the
  [Open VSX Registry](https://open-vsx.org/extension/google/jiminy-cli-vscode-ide-companion).
  Follow your editor's instructions for installing extensions from this
  registry.

<!-- prettier-ignore -->
> [!NOTE]
> The "Jiminy CLI Companion" extension may appear towards the bottom of
...
When enabled, Jiminy CLI will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **Jiminy CLI: Accept Diff**.
...
- Open the Command Palette and run **Jiminy CLI: Close Diff Editor**.
...
Jiminy CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update Jiminy CLI directly within supporting IDEs and
...
Jiminy CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install Jiminy CLI
directly through their in-built registry features.

## Using with sandboxing

If you are using Jiminy CLI within a sandbox, be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Jiminy CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Jiminy CLI could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **Jiminy CLI Companion** extension in
...
If automatic IDE detection fails, or if you are running Jiminy CLI in a
...
When this variable is set, Jiminy CLI will skip automatic detection and attempt
to connect using the provided PID.

#### Configuration errors

- **Message:**
  `🔴 Disconnected: Directory mismatch. Jiminy CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Jiminy CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Jiminy CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Jiminy CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install Jiminy CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Jiminy CLI
    Companion", and
