...
The `run_shell_command` tool allows the Jiminy model to execute commands
...
`settings.json` file or by using the `/settings` command in the Jiminy CLI.
...
When an interactive command is running, you can send input to it from the Jiminy
...
tools to detect if they are being run from within the Jiminy CLI.
