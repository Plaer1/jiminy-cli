...
The `get_internal_docs` tool lets Jiminy CLI access its own technical
documentation to provide more accurate answers about its capabilities and usage.

## Description

This tool is used when Jiminy CLI needs to verify specific details about Jiminy
...
The `get_internal_docs` tool is used exclusively by Jiminy CLI. You cannot
invoke this tool manually.

When Jiminy CLI uses this tool, it retrieves the content of the requested
...
Jiminy CLI uses this tool to ensure technical accuracy:

- **Capability discovery:** If Jiminy CLI is unsure how a feature works, it can
  lookup the corresponding documentation.
- **Reference lookup:** Jiminy CLI can verify slash command sub-commands or
  specific setting names.
- **Self-correction:** Jiminy CLI can use the documentation to correct its
  understanding of Jiminy CLI's system logic.
