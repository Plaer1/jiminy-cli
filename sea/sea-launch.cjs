...
function verifyIntegrity(dir, manifest, fsMod = fs, cryptoMod = crypto) {
  try {
    ...
    if (calculateHash(path.join(dir, 'jiminy.mjs')) !== manifest.mainHash)
      return false;
    if (manifest.files) {
...
function prepareRuntime(manifest, getAssetFn, deps = {}) {
  ...
  if (process.platform === 'win32' && processEnv.LOCALAPPDATA) {
    const appDir = pathMod.join(processEnv.LOCALAPPDATA, 'Google', 'JiminyCLI');
    try {
  ...
  const finalRuntimeDir = pathMod.join(
    tempBase,
    `jiminy-runtime-${safeVersion}-${safeUsername}`,
  );
  ...
  if (!useExisting) {
    const setupDir = pathMod.join(
      tempBase,
      `jiminy-setup-${processPid}-${Date.now()}`,
    );
    ...
    try {
      ...
      writeToSetup('jiminy.mjs', 'jiminy.mjs');
      if (manifest.files) {
        for (const file of manifest.files) {
...
async function main(getAssetFn = getAsset) {
  ...
  const mainPath = path.join(runtimeDir, 'jiminy.mjs');
