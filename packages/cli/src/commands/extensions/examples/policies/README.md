...
to the Jiminy CLI Policy Engine.
...
- `jiminy-extension.json`: The manifest file.
- `policies/`: Contains the `.toml` policy files.

## How to use

1.  Link this extension to your local Jiminy CLI installation:

    ```bash
    jiminy extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your Jiminy CLI session.
...
For security, Jiminy CLI ignores any `allow` decisions or `yolo` mode
