...
import { writeToStderr } from '@plaer1/jiminy-cli-core';
...
describe('resolveWorkspacePolicyState', () => {
  ...
  beforeEach(() => {
    ...
    // Redirect JIMINY_CLI_HOME to the temp directory to isolate integrity storage
    vi.stubEnv('JIMINY_CLI_HOME', tempDir);

    workspaceDir = path.join(tempDir, 'workspace');
    fs.mkdirSync(workspaceDir);
    policiesDir = path.join(workspaceDir, '.jiminy', 'policies');
  ...
  it('should not return workspace policies if cwd is the home directory', async () => {
    const policiesDir = path.join(tempDir, '.jiminy', 'policies');
    fs.mkdirSync(policiesDir, { recursive: true });
    fs.writeFileSync(path.join(policiesDir, 'policy.toml'), 'rules = []');
