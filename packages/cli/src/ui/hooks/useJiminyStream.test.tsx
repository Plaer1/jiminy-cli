...
import { useJiminyStream } from './useJiminyStream.js';
...
const MockedJiminyClientClass = vi.hoisted(() =>
  vi.fn().mockImplementation(function (this: any, _config: any) {
    ...
    this.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('jiminy-2.0-flash-exp');
  }),
);
...
vi.mock('@google/jiminy-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    GitService: vi.fn(),
    JiminyClient: MockedJiminyClientClass,
...
// --- Tests for useJiminyStream Hook ---
describe('useJiminyStream', () => {
  ...
  const mockGetJiminyClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedJiminyClientClass(mockConfig);
  ...
  const mockConfig: Config = {
    ...
    getJiminyClient: mockGetJiminyClient,
  ...
  beforeEach(() => {
    ...
    // Reset mocks for JiminyClient instance methods (startChat and sendMessageStream)
    // The JiminyClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // JiminyChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.jiminy/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = async (
    ...
    const client = jiminyClient || mockConfig.getJiminyClient();
          ...
          lastToolCalls = lastToolCalls.map((tc) => {
            if (
              ...
              return {
                ...
                responseSubmittedToJiminy: true,
  ...
  const renderHookWithDefaults = async (
    ...
    return renderHookWithProviders(() =>
      useJiminyStream(
        new MockedJiminyClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should inject steering hint prompt for continuation', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerJiminyEventType.Content,
    ...
    expect(mockRunInDevTraceSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: JiminyCliOperation.SystemPrompt,
      }),
      expect.any(Function),
    );
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedJiminyClientClass(mockConfig);
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerJiminyEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Jiminy when a command is handled without a tool call', async () => {
    ...
    it('should call Jiminy with prompt content when slash command returns a `submit_prompt` action', async () => {
    ...
    it('should record client-initiated tool calls in JiminyChat history', async () => {
      const { result, client: mockJiminyClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to JiminyChat history
      expect(mockJiminyClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls (like save_memory) in history', async () => {
      ...
      expect(mockJiminyClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = await renderHookWithProviders(() =>
        useJiminyStream(
          new MockedJiminyClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerJiminyEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerJiminyEventType.ContextWindowWillOverflow,
                value: {
    ...
    it('should add informational messages when ChatCompressed event is received', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerJiminyEventType.ChatCompressed,
            value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerJiminyEventType.ToolCallRequest,
    ...
    // Expectation: addItem:jiminy (rationale) MUST happen before scheduleToolCalls_START
    const rationaleIndex = addItemOrder.indexOf('addItem:jiminy');
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the Jiminy stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerJiminyEventType.Content,
  ...
  describe('Thought Reset', () => {
    it('should keep full thinking entries in history when mode is full', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerJiminyEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerJiminyEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerJiminyEventType.Error,
    ...
    it('should update lastOutputTime on Jiminy thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Loop Detection Confirmation', () => {
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerJiminyEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getJiminyClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerJiminyEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerJiminyEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerJiminyEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerJiminyEventType.Content, value: '   content' };
        })(),
      );
  ...
  it('should trace UserPrompt telemetry on submitQuery', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield { type: ServerJiminyEventType.Content, value: 'Response' };
      })(),
    );
    ...
    const userPromptCall = mockRunInDevTraceSpan.mock.calls.find(
      (call) =>
        call[0].operation === JiminyCliOperation.UserPrompt ||
