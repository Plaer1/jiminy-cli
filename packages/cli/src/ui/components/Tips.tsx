...
export const Tips: React.FC<TipsProps> = ({ config }) => {
  const jiminyMdFileCount = config.getJiminyMdFileCount();
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      ...
      <Text color={theme.text.primary}>
        {jiminyMdFileCount === 0 ? '2.' : '1.'}{' '}
        <Text color={theme.text.secondary}>/help</Text> for more information
      </Text>
      <Text color={theme.text.primary}>
        {jiminyMdFileCount === 0 ? '3.' : '2.'} Ask coding questions, edit code
        or run commands
      </Text>
      <Text color={theme.text.primary}>
        {jiminyMdFileCount === 0 ? '4.' : '3.'} Be specific for the best results
      </Text>
    </Box>
  );
};
