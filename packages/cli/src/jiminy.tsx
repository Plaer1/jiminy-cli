...
export async function main() {
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: --allowed-tools cli argument and tools.allowed in settings.json are deprecated and will be removed in 1.0: Migrate to Policy Engine: https://jiminycli.com/docs/core/policy-engine/',
    );
  }
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: tools.exclude in settings.json is deprecated and will be removed in 1.0. Migrate to Policy Engine: https://jiminycli.com/docs/core/policy-engine/',
    );
  }
  ...
  // to run Jiminy CLI. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    if (!input) {
      debugLogger.error(
        `No input provided via stdin. Input can be provided by piping data into jiminy or using the --prompt option.`,
      );
      await runExitCleanup();
      process.exit(ExitCodes.FATAL_INPUT_ERROR);
    }
