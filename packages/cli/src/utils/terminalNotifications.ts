...
import { debugLogger, writeToStdout } from '@google/jiminy-cli-core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'Jiminy CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open Jiminy CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'Jiminy CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open Jiminy CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'Jiminy CLI session complete',
