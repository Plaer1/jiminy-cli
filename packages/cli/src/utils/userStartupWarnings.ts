  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    try {
      ...
      if (workspaceRealPath === homeRealPath) {
        ...
        return 'Warning you are running Jiminy CLI in your home directory.\nThis warning can be disabled in /settings';
      }
      return null;
    } catch {
...
const rootDirectoryCheck: WarningCheck = {
  ...
  check: async (workspaceRoot: string, _settings: Settings) => {
    try {
      ...
      const errorMessage =
        'Warning: You are running Jiminy CLI in the root directory. Your entire folder structure will be used for context. It is strongly recommended to run in a project-specific directory.';
  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    if (isHeadlessMode()) {
      throw new FatalUntrustedWorkspaceError(
        'Jiminy CLI is not running in a trusted directory. To proceed, either use `--skip-trust`, ' +
          'set the `GEMINI_CLI_TRUST_WORKSPACE=true` environment variable, or trust this directory in interactive mode. ' +
          'For more details, see https://jiminycli.com/docs/cli/trusted-folders/#headless-and-automated-environments',
      );
    }
