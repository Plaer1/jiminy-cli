...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-jiminy-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-jiminy-cli release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest run-jiminy-cli release on GitHub.`,
    );
  }
};
