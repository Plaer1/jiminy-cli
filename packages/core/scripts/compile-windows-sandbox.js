...
/**
 * Compiles the JiminySandbox C# helper on Windows.
...
function compileWindowsSandbox() {
  ...
  const srcHelperPath = path.resolve(
    __dirname,
    '../src/sandbox/windows/JiminySandbox.exe',
  );
  const distHelperPath = path.resolve(
    __dirname,
    '../dist/src/sandbox/windows/JiminySandbox.exe',
  );
  const sourcePath = path.resolve(
    __dirname,
    '../src/sandbox/windows/JiminySandbox.cs',
  );
  ...
  if (result.status === 0) {
    console.log('Successfully compiled JiminySandbox.exe to src');
    ...
    if (fs.existsSync(distDir)) {
      ...
      console.log('Successfully copied JiminySandbox.exe to dist');
    }
  } else {
