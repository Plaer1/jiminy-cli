...
export class Logger {
  private jiminyDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.jiminyDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.jiminyDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.jiminyDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.jiminyDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.jiminyDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.jiminyDir) {
      debugLogger.error(
