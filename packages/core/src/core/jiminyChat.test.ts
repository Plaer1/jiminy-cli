...
describe('JiminyChat', () => {
  ...
  beforeEach(() => {
    ...
    let currentModel = 'jiminy-pro';
    let currentActiveModel = 'jiminy-pro';
    ...
    mockConfig = {
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((modelConfigKey) => {
          ...
          const thinkingConfig = model.startsWith('jiminy-3')
            ? {
  ...
  describe('sendMessageStream', () => {
    ...
    it('should use thinkingLevel and remove thinkingBudget for jiminy-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
      ...
      const stream = await chat.sendMessageStream(
        { model: 'jiminy-3-test-only-model-string-for-testing' },
        'hello',
    ...
    it('should use thinkingBudget and remove thinkingLevel for non-jiminy-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
    ...
    it('should flush transcript before tool dispatch for pure tool call with no text or thoughts', async () => {
      ...
      const jiminyMessages = lastWriteData.messages.filter(
        (m) => m.type === 'jiminy',
      );
      expect(jiminyMessages.length).toBeGreaterThan(0);
    });
  });
  ...
  describe('sendMessageStream with retries', () => {
    it('should not retry on invalid content if model does not start with jiminy-2', async () => {
  ...
  describe('Availability Service Integration', () => {
    ...
    it('should pass attempted model to onPersistent429 callback which calls handleFallback', async () => {
      ...
      // handleFallback is called with the ATTEMPTED model (model-a), not the requested one (jiminy-pro)
      expect(mockHandleFallback).toHaveBeenCalledWith(
        expect.anything(),
        'model-a',
        expect.anything(),
