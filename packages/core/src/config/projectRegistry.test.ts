...
describe('ProjectRegistry', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'jiminy-registry-test-'));
  ...
  it('handles collisions with unique suffixes', async () => {
    ...
    const id1 = await registry.getShortId(path.join(tempDir, 'one', 'jiminy'));
    const id2 = await registry.getShortId(path.join(tempDir, 'two', 'jiminy'));
    const id3 = await registry.getShortId(
      path.join(tempDir, 'three', 'jiminy'),
    );

    expect(id1).toBe('jiminy');
    expect(id2).toBe('jiminy-1');
    expect(id3).toBe('jiminy-2');
  });
  ...
  it('handles collisions if a slug is taken on disk by another project', async () => {
    // 1. project-y takes 'jiminy' on disk
    ...
    // 2. project-z tries to get shortId for 'jiminy'
    const registry = new ProjectRegistry(registryPath, [baseDir1]);
    await registry.initialize();
    const projectZ = normalizePath(path.join(tempDir, 'jiminy'));
    const shortId = await registry.getShortId(projectZ);

    // 3. It should avoid 'jiminy' and pick 'jiminy-1' (or similar)
    expect(shortId).not.toBe('jiminy');
    expect(shortId).toBe('jiminy-1');
  });
