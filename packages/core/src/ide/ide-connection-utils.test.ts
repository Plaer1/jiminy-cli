  ...
  describe('getConnectionConfigFromFile', () => {
    it('should return config from the specific pid file if it exists', async () => {
      ...
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'jiminy', 'ide', 'jiminy-ide-server-12345.json'),
        'utf8',
      );
    });
    ...
    it('should find and parse a single config file with the new naming scheme', async () => {
      ...
      (
        vi.mocked(fs.promises.readdir) as Mock<
          (path: fs.PathLike) => Promise<string[]>
        >
      ).mockResolvedValue(['jiminy-ide-server-12345-123.json']);
      vi.mocked(fs.promises.readFile).mockResolvedValue(JSON.stringify(config));
      ...
      expect(result).toEqual(config);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'jiminy', 'ide', 'jiminy-ide-server-12345-123.json'),
        'utf8',
      );
    });
    ...
    it('should filter out configs with invalid workspace paths', async () => {
      ...
      (
        ...
        'jiminy-ide-server-12345-111.json',
        'jiminy-ide-server-12345-222.json',
      ]);
    ...
    it('should ignore files with invalid names', async () => {
      ...
      (
        ...
        'jiminy-ide-server-12345-111.json', // valid
        'not-a-config-file.txt', // invalid
        'jiminy-ide-server-asdf.json', // invalid
      ]);
      vi.mocked(fs.promises.readFile).mockResolvedValueOnce(
        JSON.stringify(validConfig),
      );
      ...
      expect(result).toEqual(validConfig);
      expect(fs.promises.readFile).toHaveBeenCalledWith(
        path.join('/tmp', 'jiminy', 'ide', 'jiminy-ide-server-12345-111.json'),
        'utf8',
      );
      expect(fs.promises.readFile).not.toHaveBeenCalledWith(
        path.join('/tmp', 'jiminy', 'ide', 'not-a-config-file.txt'),
        'utf8',
      );
    });
