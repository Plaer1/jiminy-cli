...
import { getCurrentJiminyMdFilename } from '../tools/memoryTool.js';
...
export interface ExcludeOptions {
  ...
  /**
   * Whether to include dynamic patterns like the current Jiminy MD filename. Defaults to true.
   */
...
export class FileExclusions {
  ...
  getDefaultExcludePatterns(options: ExcludeOptions = {}): string[] {
    ...
    // Add dynamic patterns (like current Jiminy MD filename)
    if (includeDynamicPatterns) {
      patterns.push(`**/${getCurrentJiminyMdFilename()}`);
    }
