...
export const MEMORY_SECTION_HEADER = '## Jiminy Added Memories';

// This variable will hold the currently configured filename for GEMINI.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setJiminyMdFilename.
let currentJiminyMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setJiminyMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentJiminyMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentJiminyMdFilename = newFilename.trim();
  }
}

export function getCurrentJiminyMdFilename(): string {
  if (Array.isArray(currentJiminyMdFilename)) {
    return currentJiminyMdFilename[0];
  }
  return currentJiminyMdFilename;
}
...
export function getGlobalMemoryFilePath(): string {
  return path.join(Storage.getGlobalJiminyDir(), getCurrentJiminyMdFilename());
}
