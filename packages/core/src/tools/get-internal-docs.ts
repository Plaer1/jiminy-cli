...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Jiminy CLI documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Jiminy CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Jiminy CLI's internal documentation.
