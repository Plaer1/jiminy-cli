...
const cliConfig = {
  ...
  entryPoints: { jiminy: 'packages/cli/index.ts' },
  outdir: 'bundle',
  splitting: true,
  define: {
...
Promise.allSettled([
  ...
  if (cliResult.status === 'rejected') {
    console.error('jiminy.js build failed:', cliResult.reason);
    process.exit(1);
  }
  // error in a2a-server bundling will not stop jiminy.js bundling process
